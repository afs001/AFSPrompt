...
class Qid2Data(Dict):
    def __init__(self, __C, splits, annotated=False, similar_examples=None):
        ...
        if annotated:
            ...
            for split in splits:
                ...
                _anno_set = json.load(open(split_path, 'r', encoding="utf-8"))
        ...
        qid_to_knowledge = json.load(open(__C.KNOWLEDGE_PATH))

        iid_to_capt = json.load(open(__C.CAPTIONS_PATH))

        # iid_to_tag = json.load(open(__C.TAGS_PATH))
        ...
        for qid in qid_to_ques:
            ...
            if "ok" == __C.TASK:
                knowledge = qid_to_knowledge[qid]
            else:
                knowledge = None

            iid = str(q_item['image_id'])
            caption = iid_to_capt[iid]
            if isinstance(caption, str):
                caption = caption.strip()
                if caption[-1] != '.':
                    caption += '.'

            # tag = iid_to_tag[iid].strip()
        ...
        if annotated:
            print(
                f'Loaded dataset size: {len(self.qid_to_data)}, top{k} accuracy: {self.topk_accuracy(k) * 100:.2f}, top1 accuracy: {self.topk_accuracy(1) * 100:.2f}')
    ...
    def get_caption(self, qid, with_tag=False):
        caption = self[qid]['caption']
        if with_tag:
            tags = self.get_tags(qid)
            caption += " " + tags
        if isinstance(caption, str):
            return caption
        else:
    ...
    def get_knowledge(self, qid):
        know = self[qid]['knowledge']
        return know
