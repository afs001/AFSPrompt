...
#     def __init__(self, in_dim, size):
#         super().__init__()
#         assert in_dim % 2 == 0, "in_dim 必须为偶数"
#         dim = in_dim // 2
...
#         _cos = sinusoid_inp.cos()
#         # 拼成 [size, dim]
#         _sin = torch.repeat_interleave(_sin, 2, dim=1)
#         _cos = torch.repeat_interleave(_cos, 2, dim=1)
#         # 拼成 [size*size, in_dim]
#         _sin = torch.cat([_sin.unsqueeze(0).repeat(size, 1, 1),
#                           _sin.unsqueeze(1).repeat(1, size, 1)], dim=-1).view(size*size, in_dim)
#         _cos = torch.cat([_cos.unsqueeze(0).repeat(size, 1, 1),
#                           _cos.unsqueeze(1).repeat(1, size, 1)], dim=-1).view(size*size, in_dim)
#         self.register_buffer('sin', _sin)
#         self.register_buffer('cos', _cos)
#
#     def forward(self, q, k):
#         # q, k: [B, N, D]
#         sin = self.sin[:q.shape[1], :].to(q.device)
#         cos = self.cos[:q.shape[1], :].to(q.device)
